...
aco <- read.table('~/TranscInterTF/data/araport/aco4.txt', stringsAsFactors = F, header = T)
...
##choose tissue to median counts and for heatmap
median_data <- function(file_name, level) {
  df <- read.csv(file_name)
  gene_median <- unlist(strsplit(file_name, '_'))[1]
  i <- 2
  for (tissue in level) {
    #a <- median(df[df$experiment_tissue == tissue])
    gene_median[i] <- median(df[df$experiment_tissue == tissue, 2])
    i <- i+1
  }
  return(gene_median)
}
...
temp_df <- median_data(aco_file[1], level_heatmap)
i <- 2
while (i <= length(aco_file)) {
  temp_df <- rbind((temp_df), median_data(aco_file[i], level_heatmap))
  i <- i+1
}
...
write.csv(genes_heat, 'tissue_specific_expr_median_aco4.csv', row.names = T, quote = F)
library(gplots)
